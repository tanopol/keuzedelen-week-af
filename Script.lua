...
function story(aName)
	...
	if(aName == "man from online") then
		...
		playSound("talk.wav")
			setBackground("inside cabin.jpeg")
			createTextfield("so what do you want?")
			createButton("bands", "I want to learn how to make bands")
			createButton("shut up", "shut the fuck up") 
	end

	if(aName == "shut up") then
		CLS()
		playSound("r2d2.wav")
			setBackground("knife attack.jpeg")
			createTextfield("HOW DARE YOU DISRESPECT ME! tou get stabbed")
			createButton("exit", "")
	end
	...
	if(aName == "bands") then
		...
		playSound("talk.wav")
			setBackground("inside cabin.jpeg")
			createTextfield("aight bruh sit down ill teach you")
			createButton("bands2", "hell yeah brotha")
			createButton("not internet man", "nah") 
	end

	if(aName == "bands2") then
		CLS()
		playSound("talk.wav")
			setBackground("inside cabin.jpeg")
			createTextfield("first you need to learn the saying")
			createButton("-", "I already know it, fuck bitches get money")
	end
	...
	if(aName == "-") then
		...
		playSound("talk.wav")
			setBackground("inside cabin.jpeg")
			createTextfield("yuhhhhh very good")
			createButton("_", "thanks, whats next?")
	end
	...
	if(aName == "_") then
		...
		playSound("talk.wav")
			setBackground("inside cabin.jpeg")
			createTextfield("nothing thats all you need to know")
			createButton("1", "wow already?!")
	end

	if(aName == "1") then
		CLS()
			setBackground("inside cabin.jpeg")
			createTextfield("yep, go get that king or whatever")
			createButton("2", "aight bet, this man is gonna loose the band for band so hard")
	end

	if(aName == "2") then
		CLS()
			setBackground("dark lord.jpeg")
			createTextfield("you go back to the dark knight to challange him")
			createButton("3", "GO BAND FOR BAND WITH ME RIGHT NOW")
	end
	...
	if(aName == "3") then
		...
		playSound("footstep.wav")
			setBackground("dark lord.jpeg")
			createTextfield("aight bruh be prepared to perish")
			createButton("4", "get scared and run away")
			createButton("5", "go band for band with him") 
	end
	...
	if(aName == "4") then
		...
		playSound("footstep.wav")
			setBackground("forest run.png")
			createTextfield("what are you a pussy? come on now")
			createButton("5", "go band for band with him after all")
	end
	...
	if(aName == "5") then
		...
		playSound("money.wav")
			setBackground("money spread.png")
			createTextfield("you pull out so many bands he instantly dies")
			createButton("6", "finally I am victorious!!!")
	end

	if(aName == "6") then
		CLS()
		playSound("yipe.wav")
			setBackground("nerd.png")
			createTextfield("you go inside his house and live hapilly ever after")
